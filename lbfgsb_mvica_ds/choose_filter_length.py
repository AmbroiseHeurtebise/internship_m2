...
def run_experiment(**params):
    ...
    # output
    filter_length_envelop = params["filter_length_envelop"]
...
if __name__ == '__main__':
    ...
    params = {
        ...
        "W_scale": 15,
        "penalization_scale": 20,
    }

    # varying params
    filter_lengths_envelop = 2 ** np.arange(7)  # array([1, 2, 4, 8, 16, 32, 64])
    nb_seeds = 30
    random_states = np.arange(nb_seeds)
    nb_expes = len(filter_lengths_envelop) * len(random_states)

    # run experiment
    print("\n############################################### Start ###############################################")
    df_res = pd.DataFrame()
    for i, (filter_length_envelop, random_state) in tqdm(enumerate(product(filter_lengths_envelop, random_states))):
        params["filter_length_envelop"] = filter_length_envelop
    ...
    gen = params["generation_function"]
    if params["A_B_init_permica"]:
        init = "_initpermica"
    else:
        init = ""
    save_name = f"DataFrame_with_{nb_seeds}_seeds_gen{gen}{init}_by_filter_length_envelop"

...
def run_experiment_wrapped(varying_output, **kwargs):
    dict_varying_outputs = {"n_concat": varying_output}
...
n_concat_all = np.arange(1, 11)
nb_expes = len(n_concat_all) * len(random_states)
...
dict_res = Parallel(n_jobs=N_JOBS)(
    ...
    ) for n_concat, random_state
    in product(n_concat_all, random_states)
)
print("\n######################################### Obtained DataFrame #########################################")
